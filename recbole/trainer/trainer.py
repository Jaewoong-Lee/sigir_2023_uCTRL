...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        saved_model_file = '{}-{}-{}.pth'.format(self.config['model'], self.config['dataset'], get_local_time()) # add dataset name
        ...
        ## jw
        self.eval_unbiased_flag = False
        if config['eval'] == 'unbiased':
            self.eval_unbiased_flag = True
    ...
    def _build_optimizer(self, **kwargs):
        ...
        if self.config['model'] == 'BCLOSS': #############################
            optimizer = optim.Adam([ param for param in params if param.requires_grad == True], lr=learning_rate)
    ...
    def _train_epoch(self, train_data, all_loss_ls, epoch_idx, loss_func=None, show_progress=False, test_data=None):
        ...
        # [ANALYSIS] count negative, false negative, high margin samples
        if test_data is not None:
            items_by_users = test_data.dataset.history_item_matrix()[0]
            total_neg_samples = 0
            total_false_neg_samples = 0
            total_high_margin_samples = 0
            total_high_margin_of_fn_samples = 0
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            # print(len(interaction.cpu().numpy()["user_id"]))
        ...
        if total_check_loss:
            ...
            all_loss_ls.append(list(total_check_loss))
            # print(all_loss_ls)
        

            # [ANALYSIS] 매 epoch마다 negative scores 계산
        if test_data is not None:
            neg_item_score = self.model.neg_scores(interaction)
            total_neg_samples += neg_item_score.numel()
            neg_item_label = np.zeros(neg_item_score.size())
            for i, (u_id, neg_ids) in enumerate(zip(interaction['user_id'], interaction['neg_item_id'])):
                items_by_user = items_by_users[u_id]
                neg_item_label[i, :] = np.array([x in items_by_user.cpu() for x in neg_ids.cpu()])
            neg_item_label = torch.tensor(neg_item_label).to(self.device)
            total_false_neg_samples += neg_item_label.sum().item()
            total_high_margin_samples += (neg_item_score >= 0.5).sum().item()
            total_high_margin_of_fn_samples += ((neg_item_label * neg_item_score) >= 0.5).sum().item()

        # [ANALYSIS] print samples
        if test_data is not None:
            print("# NegSamples: ", total_neg_samples)
            print("# FalseNegSamples: ", total_false_neg_samples)
            print("# HighMarginSamples: ", total_high_margin_samples)
            print("# FalseNegSamples with high margin: ", total_high_margin_of_fn_samples)

            print('Ratio of FN', (total_false_neg_samples / total_neg_samples)*100)
            print('Ratio of HM', (total_high_margin_samples / total_neg_samples)*100)
            print('Ratio of FN&HM ', (total_high_margin_of_fn_samples / total_neg_samples)*100)

        # [ANALYSIS]
        if test_data is not None:
            return total_loss, total_neg_samples, total_false_neg_samples, total_high_margin_samples, total_high_margin_of_fn_samples
        else:
            return total_loss
    ...
    ### jw
    def _unbiased_valid_epoch(self, valid_data, show_progress=False, pscore=None):
        r"""Valid the model with valid data
        ...
        Returns:
            ...
            dict: valid result
        """
        valid_result = self.unbiased_evaluate(valid_data, load_best_model=False, show_progress=show_progress, pscore=pscore) # 여기에서 이미 결정됨
        valid_score = calculate_valid_score(valid_result, self.valid_metric)
        return valid_score, valid_result
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None, test_data=None):
        ...
        ## jw
        pscore = train_data.dataset.eval_pscore()

        ##########################################################################
        if self.config['model'] == 'BCLOSS' :

            for epoch_idx in range(self.config['freeze_epoch'] ):
                # train
                training_start_time = time()
                train_loss = self._train_epoch(train_data, epoch_idx, loss_func=self.model.calculate_pre_loss, show_progress=show_progress)
                ...
                if (epoch_idx + 1) % self.eval_step == 0:
                    ...
                    if self.eval_unbiased_flag:
                        valid_score, valid_result = self._unbiased_valid_epoch(valid_data, show_progress=show_progress, pscore=pscore)
                        # print(valid_score)
                    else:
            ...
            self.start_epoch = self.config['freeze_epoch']
        ##########################################################################
        all_loss_ls = []
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            if self.config['model'] == 'BCLOSS' or self.config['model'] == 'uCTRL': ##########
                if epoch_idx > 5:
                    self.model.freeze_pop()
                train_loss = self._train_epoch(train_data,all_loss_ls, epoch_idx, show_progress=show_progress)
            else:
                train_loss = self._train_epoch(train_data,all_loss_ls, epoch_idx, show_progress=show_progress)
            self.train_loss_dict[epoch_idx] = sum(train_loss) if isinstance(train_loss, tuple) else train_loss
            training_end_time = time()
        ...
        ########## save uniformity, alignment code ##################
        
        
        np.save(f"all_loss_{self.config['dataset']}_{self.config['model']}.npy",np.array(all_loss_ls))
        # self.users_pop, self.items_pop = train_data.get_user_item_pop_unpop()
        # all_user_pop = self.users_pop['user_id']
        # print(all_user_pop)
        ...
        total_end_time = time()
        self.logger.info(set_color("Total (train & valid) time", 'blue') + ": %.2fs" % (total_end_time - total_start_time))
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            interaction, scores, positive_u, positive_i = eval_func(batched_data) # print(interaction)
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
    ...
    ## jw
    @torch.no_grad()
    def unbiased_evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False, pscore=None):
        r"""Evaluate the model based on the eval data.
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
            self.eval_collector.eval_batch_collect_unbiased(scores, interaction, positive_u, positive_i, pscore) ####### 여기서 만듦 top k에 몇개 아이템 hit하엿는지 이게 어떻게 만들어졋는지 보기
        self.eval_collector.model_collect(self.model)
        struct = self.eval_collector.get_data_struct_unbiased() ##### 이미 struct 만들 때 각 user 별로 top k에 hit를 몇 개의 item이 하였는 지를 struct에서 갖고 있음 + #### eval metric을 unbiased를 자연스럽게 불러올 수 잇는지 봐야함
        result = self.evaluator.evaluate_unbiased(struct)
        self.wandblogger.log_eval_metrics(result, head='eval')
