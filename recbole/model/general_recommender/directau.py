...
class DirectAU(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        # print(self.users_pop.sum())
        # print(self.items_pop.sum())
    ...
    def calculate_loss(self, interaction):
        ...
        ratio = self.n_users / self.n_items
        uniform = self.gamma * (1/ratio * self.uniformity(user_e) + self.uniformity(item_e)) / 2
    ...
    def save_params(self):
        ...
        np.save(f'user_{self.model_name}_{self.dataset}.npy', user_embeddings.data.cpu().numpy())
        np.save(f'item_{self.model_name}_{self.dataset}.npy', item_embeddings.data.cpu().numpy())
        np.save(f'user_label_{self.model_name}_{self.dataset}.npy', np.array(self.users_pop.to('cpu')))
    
    ############### uniformity, alignment 저장 #################
    def check(self, interaction):
        ...
        user_popularity = self.users_pop[user].bool()
        item_popularity = self.items_pop[item].bool()
        
        upop_user_e = user_e[user_popularity]
        upop_item_e = item_e[user_popularity] 
        
        Uunpop_user_e = user_e[~user_popularity]
        Uunpop_item_e = item_e[~user_popularity] 
        
        # print(len(upop_user_e), len(Uunpop_user_e))
        
        ipop_user_e = user_e[item_popularity]
        ipop_item_e = item_e[item_popularity]
        
        iunpop_user_e = user_e[~item_popularity]
        iunpop_item_e = item_e[~item_popularity]
    
        alignment_loss = self.alignment(user_e, item_e)
        uniform_loss = (self.uniformity(user_e) + self.uniformity(item_e)) / 2
        
        alignment_loss_upop = self.alignment(upop_user_e, upop_item_e)
        uniform_loss_upop = (self.uniformity(upop_user_e) + self.uniformity(upop_item_e)) / 2
        
        alignment_loss_uun = self.alignment(Uunpop_user_e, Uunpop_item_e)
        uniform_loss_uun = (self.uniformity(Uunpop_user_e) + self.uniformity(Uunpop_item_e)) / 2
        
        alignment_loss_ipop = self.alignment(ipop_user_e, ipop_item_e)
        uniform_loss_ipop = (self.uniformity(ipop_user_e) + self.uniformity(ipop_item_e)) / 2
        
        alignment_loss_iunpop = self.alignment(iunpop_user_e, iunpop_item_e)
        uniform_loss_iunpop = (self.uniformity(iunpop_user_e) + self.uniformity(iunpop_item_e)) / 2
    
        return alignment_loss, uniform_loss, alignment_loss_upop, uniform_loss_upop, alignment_loss_uun, uniform_loss_uun, alignment_loss_ipop, uniform_loss_ipop, alignment_loss_iunpop, uniform_loss_iunpop
