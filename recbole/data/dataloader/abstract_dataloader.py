...
class NegSampleDataLoader(AbstractDataLoader):
    ...
    def _set_neg_sample_args(self, config, dataset, dl_format, neg_sample_args):
        self.uid_field = dataset.uid_field # 'user_id'
        ...
        if self.neg_sample_args['strategy'] == 'by':
            ...
            elif self.dl_format == InputType.SETWISE:
                self.times = self.neg_sample_num
                self.sampling_func = self._neg_sample_by_set_wise_sampling
    ...
    def _neg_sampling(self, inter_feat):
        if 'dynamic' in self.neg_sample_args.keys() and self.neg_sample_args['dynamic'] != 'none':
            ...
            self.model.train()
            return self.sampling_func(inter_feat, neg_item_ids) # return Interaction with negative items
        elif 'in_batch' in self.neg_sample_args.keys() and  self.neg_sample_args['in_batch']:
            user_ids = inter_feat[self.uid_field].numpy()
            item_ids = inter_feat[self.iid_field].numpy()

            batch_size = len(user_ids)
            self.times = batch_size - 1

            # TODO 2 : point-wise, pair-wise에도 동일하게 적용해야함.
            # neg_item_ids[i] = item_ids[user_ids != user_id]
            
            # Method 1
            # neg_item_ids = np.zeros((batch_size, self.times), dtype=np.int64)
            # num_neg_items = np.zeros(batch_size, dtype=np.int64)
            # # user가 평가한 item은 제외함
            # for i, used_ids_by_user in enumerate(self.sampler.used_ids[user_ids]):
            #     neg_items_by_user = set(item_ids) - used_ids_by_user
            #     num_neg_items_by_user = len(neg_items_by_user)
            #     neg_item_ids[i, :num_neg_items_by_user] = np.array(list(neg_items_by_user))

            #     # negative items 수 저장
            #     num_neg_items[i] = num_neg_items_by_user

            # Method 2
            # diagnoal elements만 제거함.
            neg_item_ids = np.tile(item_ids, (batch_size, 1), dtype=np.int64)
            neg_item_ids = neg_item_ids[~np.eye(neg_item_ids.shape[0], dtype=bool)].reshape(neg_item_ids.shape[0], -1)

            neg_item_ids = torch.tensor(neg_item_ids)
            num_neg_items = torch.tensor(num_neg_items)
            num_neg_items = None

            return self.sampling_func(inter_feat, neg_item_ids, num_neg_items)
    ...
    def _neg_sample_by_set_wise_sampling(self, inter_feat, neg_item_ids, num_neg_items=None):
        if neg_item_ids.ndim == 1:
            neg_item_ids = neg_item_ids.view(self.times, -1).T

        if num_neg_items is None:
            pass
        else:
            # negative items 수 Interaction에 넣기
            num_neg_item_feat = Interaction({'num_neg_items': num_neg_items})
            inter_feat.update(num_neg_item_feat)
    ...
    def _neg_sample_by_pair_wise_sampling(self, inter_feat, neg_item_ids):
        if neg_item_ids.ndim == 2:
            neg_item_ids = neg_item_ids.nonzero(as_tuple=True)[0]
