...
class Dataset:
    ...
    def eval_pscore(self):
        ...
        pscore = torch.ones(self.item_num).long()
        pscore[pscore_id] = pscore_cnt
        pscore = pscore**1.5 # pop^{(1+2)/2} gamma = 2
    ...
    def estimate_pscore(self):
        ...
        pscore_cnt_full[pscore_id] = pscore_cnt
        pscore_cnt_full = pow(pscore_cnt_full / pscore_cnt_full.max(), self.eta)
        pscore_cnt_i = pscore_cnt_full
        
        return pscore_cnt_u, pscore_cnt_i
    ...
    def get_user_item_pop_unpop(self):
        ...
        sorted_idx = torch.argsort(item_pop_cnt) #  값 5 1 3           idx 1 2 0
        unpop_threshold = int(len(sorted_idx)*0.8)-1   # 1
