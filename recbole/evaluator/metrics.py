...
class Recall(TopkMetric):
    ...
    def calculate_metric(self, dataobject, unbiased=False):
        if unbiased:
            pos_index, pos_len = self.used_info(dataobject)
            pscore = dataobject.get('data.pscore')
            pscore = pscore.numpy()
            pscore_all = dataobject.get('data.pscore_all')
            pscore_all = pscore_all.numpy()
            result = self.metric_info_unbiased(pos_index, pos_len, pscore, pscore_all)
    ...
    def metric_info_unbiased(self, pos_index, pos_len, pscore, pscore_all):
        result = np.cumsum(np.where(pos_index, 1/pscore, 0), axis=1) / pos_len.reshape(-1, 1)
        # result = result / np.cumsum(np.where(pos_index, pscore, 10e-3), axis=1)
        result = result
        # result = result / np.cumsum(pscore, axis=1)
        return result
...
class NDCG(TopkMetric):
    ...
    def metric_info_unbiased(self, pos_index, pos_len, pscore, pscore_all):
        len_rank = np.full_like(pos_len, pos_index.shape[1])
        idcg_len = np.where(pos_len > len_rank, len_rank, pos_len) # 최대 top N개로 pos수도 제한
        iranks = np.zeros_like(pos_index, dtype=np.float)
        iranks[:, :] = np.arange(1, pos_index.shape[1] + 1) # 모든 top N items을 1부터 N까지로 바꿔줌

        idcg = np.cumsum(1.0 / np.log2(iranks + 1), axis=1)
        for row, idx in enumerate(idcg_len): ### 이 게 뭔지 모르겟네
            idcg[row, idx:] = idcg[row, idx - 1]
            
        ranks = np.zeros_like(pos_index, dtype=np.float)
        ranks[:, :] = np.arange(1, pos_index.shape[1] + 1)
        dcg = 1.0 / (np.log2(ranks + 1) * pscore)
