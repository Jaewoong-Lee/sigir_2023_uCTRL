...
class Collector(object):
    ...
    ## jw
    def eval_batch_collect_unbiased(
        self, scores_tensor: torch.Tensor, interaction, positive_u: torch.Tensor, positive_i: torch.Tensor, pscore: torch.Tensor
    ):
        """ Collect the evaluation resource from batched eval data and batched model output.
            Args:
                scores_tensor (Torch.Tensor): the output tensor of model with the shape of `(N, )`
        ...
        if self.register.need('rec.topk'):
            ...
            pos_len_list = pos_matrix.sum(dim=1, keepdim=True)
            pos_idx = torch.gather(pos_matrix, dim=1, index=topk_idx)
            result = torch.cat((pos_idx, pos_len_list), dim=1) # positive로 예측한 item index의 실제 값(pos_idx)과 맨 마지막에 실제 positive 수(pos_len_list)
            self.data_struct.update_tensor('rec.topk', result)

            ##### pscore
            pscore_result = torch.zeros_like(pos_idx, dtype=torch.float)
            self.device = torch.device("cuda")
            pscore_result = pscore_result.to(self.device)
            pscore = pscore.to(self.device)
            pscore_result[:, :] = pscore[topk_idx]
            self.data_struct.update_tensor('data.pscore', pscore_result)

            ##### pscore all
            pos_matrix_tmp = torch.zeros_like(scores_tensor, dtype=torch.float)
            pos_matrix_tmp[positive_u, positive_i] = pscore[positive_i].to(self.device)
            pscore_result_all_user = pos_matrix_tmp.sum(dim=1, keepdim=True)
            self.data_struct.update_tensor('data.pscore_all', pscore_result_all_user)
    ...
    def get_data_struct_unbiased(self):
        """ Get all the evaluation resource that been collected.
            And reset some of outdated resource.
        """
        returned_struct = copy.deepcopy(self.data_struct)
        for key in ['rec.topk', 'rec.meanrank', 'rec.score', 'rec.items', 'data.label', 'data.pscore', 'data.pscore_all']:
            if key in self.data_struct:
                del self.data_struct[key]
        return returned_struct
